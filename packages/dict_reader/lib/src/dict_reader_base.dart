...
class DictReader {
  ...
  /// Exports the cache data in a compact binary format.
  ///
  /// Much faster to load than the JSON produced by [exportCacheAsString]:
  /// no JSON parsing is involved and the result is transferred out of the
  /// isolate without a deep copy. See [importCacheFromBinary].
  Future<Uint8List> exportCacheAsBinary() {
    final keyList = _keyList;
    final numEntries = this.numEntries;
    final recordBlockOffset = _recordBlockOffset;
    final recordBlockInfoList = _recordBlockInfoList;
    final totalDecompressedSize = _totalDecompressedSize;
    return Isolate.run(() => _encodeBinaryCache(keyList, numEntries,
        recordBlockOffset, recordBlockInfoList, totalDecompressedSize));
  }

  /// Imports cache data produced by [exportCacheAsBinary].
  ///
  /// Decoding happens in an isolate; the decoded structures are moved back
  /// via `Isolate.exit` without a deep copy.
  Future<void> importCacheFromBinary(Uint8List bytes) async {
    final data = await Isolate.run(() => _decodeBinaryCache(bytes));
    _keyList = data.keyList;
    numEntries = data.numEntries;
    _recordBlockOffset = data.recordBlockOffset;
    _recordBlockInfoList = data.recordBlockInfoList;
    _totalDecompressedSize = data.totalDecompressedSize;
  }
...
/// Decoded result of the binary cache format (see [_encodeBinaryCache]).
class _BinaryCacheData {
  final List<(int, String)> keyList;
  ...
  _BinaryCacheData(this.keyList, this.numEntries, this.recordBlockOffset,
      this.recordBlockInfoList, this.totalDecompressedSize);
}

const _binaryCacheMagic = 0x44524231; // "DRB1"

Uint8List _encodeBinaryCache(
    List<(int, String)> keyList,
    int numEntries,
    int recordBlockOffset,
    List<(int, int)>? recordBlockInfoList,
    int? totalDecompressedSize) {
  final encodedKeys = List<Uint8List>.generate(
      keyList.length, (i) => utf8.encode(keyList[i].$2),
      growable: false);

  var size = 4 + 8 + 8 + 1 + 8 + 1 + 8;
  if (recordBlockInfoList != null) {
    size += recordBlockInfoList.length * 16;
  }
  size += 8;
  for (final bytes in encodedKeys) {
    size += 8 + 4 + bytes.length;
  }
  ...
  view.setUint32(offset, _binaryCacheMagic);
  offset += 4;
  view.setInt64(offset, numEntries);
  offset += 8;
  view.setInt64(offset, recordBlockOffset);
  offset += 8;
  view.setUint8(offset, totalDecompressedSize == null ? 0 : 1);
  offset += 1;
  view.setInt64(offset, totalDecompressedSize ?? 0);
  offset += 8;
  view.setUint8(offset, recordBlockInfoList == null ? 0 : 1);
  offset += 1;
  view.setInt64(offset, recordBlockInfoList?.length ?? 0);
  offset += 8;
  if (recordBlockInfoList != null) {
    for (final (a, b) in recordBlockInfoList) {
      view.setInt64(offset, a);
      offset += 8;
      view.setInt64(offset, b);
      offset += 8;
    }
  }
  view.setInt64(offset, keyList.length);
  offset += 8;
  for (var i = 0; i < keyList.length; i++) {
    view.setInt64(offset, keyList[i].$1);
    offset += 8;
    final bytes = encodedKeys[i];
...
_BinaryCacheData _decodeBinaryCache(Uint8List buffer) {
  final view = ByteData.view(buffer.buffer, buffer.offsetInBytes);
  var offset = 0;

  if (view.getUint32(offset) != _binaryCacheMagic) {
    throw FormatException("Invalid binary cache format");
  }
  offset += 4;
  final numEntries = view.getInt64(offset);
  offset += 8;
  final recordBlockOffset = view.getInt64(offset);
  offset += 8;
  final hasTotalDecompressedSize = view.getUint8(offset) == 1;
  offset += 1;
  final totalDecompressedSize = view.getInt64(offset);
  offset += 8;
  final hasRecordBlockInfo = view.getUint8(offset) == 1;
  offset += 1;
  final recordBlockInfoCount = view.getInt64(offset);
  offset += 8;
  List<(int, int)>? recordBlockInfoList;
  if (hasRecordBlockInfo) {
    recordBlockInfoList = List<(int, int)>.generate(recordBlockInfoCount, (i) {
      final a = view.getInt64(offset);
      final b = view.getInt64(offset + 8);
      offset += 16;
      return (a, b);
    }, growable: false);
  }
  final keyCount = view.getInt64(offset);
  offset += 8;
  final keyList = List<(int, String)>.generate(keyCount, (i) {
    final recordStart = view.getInt64(offset);
    offset += 8;
    ...
    offset += length;
    return (recordStart, key);
  }, growable: false);

  return _BinaryCacheData(
      keyList,
      numEntries,
      recordBlockOffset,
      recordBlockInfoList,
      hasTotalDecompressedSize ? totalDecompressedSize : null);
}
